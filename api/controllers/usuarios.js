...
module.exports = {
  ...
  async cadastrarUsuario(request, response) {
    try {
      const { email, pass, nome, telefone, nascimento, rua, num, bairo, cidade, estado, cep } = request.body;
      if (email && pass && nome && telefone && nascimento && rua && num && bairo && cidade && estado && cep) {
        ...
        if (!resEmail[0][0].email) {
          const sqlEndereco = "INSERT INTO enderecos( end_rua, end_num, end_bairo, end_cidade, end_estado, end_cep) VALUES ( ?, ?, ?, ?, ?, ? )";
          const resCadastrarEndereco = await db.query(sqlEndereco , [ rua, num, bairo, cidade, estado, cep ]);
